...
def f(s,a, net):
    ...
    u_ss = T.autograd.grad(u_s.sum(), s_sample, create_graph=True)[0]
    u_ss = u_ss.abs().sum(1,True)
    u_aaa = T.autograd.grad(u_aa.sum(), a_sample, create_graph=True)[0]
    u_aaa = u_aaa.abs().sum(1,True)
    u_aaaa = T.autograd.grad(u_aaa.sum(), a_sample, create_graph=True)[0]
    u_aaaa = u_aaaa.abs().sum(1,True)
    
    
    
    
    pde = u_s + u*u_a - (0.01/pi)*u_aa #training on 14 october
    
    #pde = u_s + u_a 
    
    #pde = u_s + u*u_a + u_aa + u_aaaa    #Kuramoto-Sivashinsky equation  train on 29/10
    
    #pde = u_s + 6*u*u_a + u_aaa  # Korteweg-de Vries (KdV) equation  train on 12/21
...
class Agent():
    ...
    def learn(self):
        ...
        self.pde = mse_f.detach().cpu().numpy()
