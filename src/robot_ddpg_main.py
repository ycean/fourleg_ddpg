...
if __name__ == '__main__':
	...
	n_games = 20000
	batch_size=1000
	filename = 'Quadrob_ddpg' + str(agent.alpha) + '_beta_' + str(agent.beta) + '_' + str(n_games) + '_games_' + str(agent.batch_size) +'_batch'
	...
	for i in range(n_games):
		...
		filename2 = 'QuadRob_ddpg_q_value_' + str(n_games) + '_games_'+ str(agent.batch_size) +'_batch.csv'
		np.savetxt(filename2, q_history, delimiter=",") # q_value
		filename3 = 'QuadRob_ddpg_target_q_value_' + str(n_games) + '_games_'+ str(agent.batch_size) +'_batch.csv'
		np.savetxt(filename3, t_q_history, delimiter=",") # target_q_value
		filename4 = 'QuadRob_ddpg_u_loss_value_' + str(n_games) + '_games_'+ str(agent.batch_size) +'_batch.csv'
